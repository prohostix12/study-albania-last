...
export default function HeroSection() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    saveEnquiry({ ...form, source: 'Hero Form' });
    setSubmitted(true);
    setTimeout(() => { setSubmitted(false); setForm({ name: '', phone: '', email: '', country: '', course: '' }); }, 4000);
  };
      ...
      <div className={`container ${styles.heroContent}`}>
        ...
        <div className={styles.heroLeft}>
          ...
          <div className={styles.heroBtns}>
            <button type="button" onClick={() => window.dispatchEvent(new CustomEvent('open-apply-modal'))} className={`btn btn-accent ${styles.heroBtn}`}>
              ✦ Apply Now — Free
            </button>
